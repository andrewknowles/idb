...
//returns query string from qry table using qry_id
		...
		{
		$queryrequest = 'select qry_title, qry_qry, qry_qry2, qry_qry3, qry_link, qry_detail from idb_query where qry_type ='.$querytype. ' order by qry_order';
//echo $queryrequest;
//exit;
			try
			{
				...
				{
					$this->openConnection();
						$this->lastQuery = mysqli_query($this->connection, $this->ecapeString($queryrequest));
					//				$this->closeConnection();
				...
				{
						$this->lastQuery = mysqli_query($this->connection, $this->escapeString($queryrequest));
					}
		...
		{
			$queryrequest = 'select qry_title, qry_qry, qry_qry2, qry_qry3, qry_link, qry_detail from idb_query where qry_id ='.$queryno ;
//			echo $queryrequest;
//			exit;
			try
			{
	...
	public function prep_query($enteredquery)
	{
		mysqli_prepare($this->connection, 'SELECT qry_id, qry_title FROM idb_query where
                      qry_id=? and qry_link < ?'); 
	}
